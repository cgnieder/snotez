...
\setlength\textwidth{\dimexpr\textwidth-\marginparwidth\relax}
\setlength{\marginparwidth}{1.75\marginparwidth}

\usepackage[footnote,perpage,dblarg]{snotez}
\setcnltx{
  package  = snotez ,
...
\defbibheading{bibliography}[\bibname]{\clearpage\addsec{References}}
...
\snotez\ needs and loads the packages \pkg*{cnltx-base} (from the \bnd{cnltx}
bundle)\sidenote[*-][]{\textcite{bnd:cnltx}},
...
afternoon\sidenote{\url{http://www.youtube.com/watch?v=PPRiaYH1iTk}}.
Well, more or less at least.  Since there already is Andy Thomas'
\pkg{sidenotes} package\sidenote{\textcite{pkg:sidenotes}} there is
probably no real need for \snotez.  Moreover, the tufte
classes\sidenote{\textcite{cls:tufte}} as well as
\cls{memoir}\sidenote{\textcite{cls:memoir}} also provide corresponding
...
\pkg{sidenotes} package such as inserting kerns and superscripted commas for
multiple marks.  I prefer to let my \pkg{fnpct}\sidenote{\textcite{pkg:fnpct}}
...
the \option{dblarg} option (see page~\pageref{key:dblarg}) when you're using
\pkg{fnpct} together with \snotez!
...
\begin{commands}
  ...
  \command{sidenote}[\oarg{offset}\oarg{mark}\marg{text}]
    ...
    \cs*{marginnote} command\sidenote[*-3][]{From the \pkg{marginnote}
      package}.  Please see its documentation\sidenote[*-2][]{For example with
    ...
    When you're using the \keyis{dblarg}{false} option the \meta{offset}
  ...
  \command{sidenotetext}[\oarg{offset}\oarg{mark}\marg{text}]
...
\begin{options}
  \keybool{dblarg}\Default{true}\label{key:dblarg}%
    \emph{This option can only be used as a package option!}  Prior to
    version~0.3 \snotez' \cs{sidenote} and \cs{sidenotetext} had both their
    ...
    option.  Setting it to \code{false} changes the \oarg{offset} argument
    syntax into \darg{offset}.
    
    If you're using \snotez\ together with the \pkg{fnpct}
    package~\cite{pkg:fnpct} you cannot set \option{dblarg} to \code{false}.
    Otherwise the \meta{offset} argument get's lost!
  ...
  \keyval{text-mark-format}{code}\Default{\cs*{@textsuperscript}\Marg{\cs*{normalfont}\#1}}
    The format of the sidenote mark in the text\sidenote[*-2][]{This document
      uses \sharpe's \pkg{superiors} package~\cite{pkg:superiors} and
